...
// Traer datos del usuario siempre
$sql = "SELECT dni, nombre, telefono, localidad, foto_perfil 
        FROM usuarios 
        WHERE dni = ?";
...
// Intentar traer ocupación si es trabajador
$sql_trab = "SELECT ocupacion, linkdepago FROM trabajador WHERE identificador = ?";
$stmt_trab = $conex->prepare($sql_trab);
$stmt_trab->bind_param("s", $dni);
$stmt_trab->execute();
$res_trab = $stmt_trab->get_result();
$trabajador = $res_trab->fetch_assoc();

if ($trabajador) {
    $datos['ocupacion'] = $trabajador['ocupacion'];
    $datos['linkdepago'] = $trabajador['linkdepago'];
} else {
    $datos['ocupacion'] = null;
    $datos['linkdepago'] = null;
}


// Cargar publicaciones
$sql_pub = "SELECT id_publicacion, foto_publicacion, descripcion 
            FROM publicacion 
            WHERE dni_usuario = ?";
...
.form-group textarea:focus {
  border-color: #5e5b64;
  outline: none;
  box-shadow: 0 0 6px rgba(94,91,100,0.3);
}
...
<div class="info-item">
    ...
    <strong>Localidad:</strong> <?php echo htmlspecialchars($datos['localidad']); ?>
</div>

<?php if (!empty($datos['ocupacion'])): ?>
<div class="info-item">
...
<a href="ver_perfil.php?dni=<?php echo urlencode($cal['dni_usuario']); ?>" 
   style="text-decoration:none; color:#333; font-weight:bold;">
   <?php echo htmlspecialchars($cal['nombre']); ?>
</a><br>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <p style="text-align:center;">Este usuario aún no tiene calificaciones.</p>
        <?php endif; ?>
    </div>
</section>
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const traducciones = {
        ...
        comentarBoton: "Comment",
            ...
            home: "Home", premium: "Premium", categorias: "Categories", bandeja: "Inbox", configuracion: "Settings", ayuda: "Help", 
...
if (botonComentar) botonComentar.textContent = t.comentarBoton;
    ...
    const btnContratar = document.querySelector('button[type="button"]');
...
// Cierra si clickea afuera
document.getElementById("modalContratar").addEventListener("click", function(e) {
  const modalContent = document.querySelector("#modalContratar .modal-content");
  if (!modalContent.contains(e.target)) {
    cerrarFormulario();
  }
});
...
<a href="chat.php?chat_dni=<?= $dni?>" class="btn-flotante" title="Chatear">
